...
import {
  InterfaceResponseBusquedaFirmas,
  InterfaceMensajeModal,
  typeMSM,
} from "../types/InterfaceResponseBusquedaFirmas";
...
*/
const WidgetSearchSIEC = (props: AllWidgetProps<any>) => {
  console.log("RIGoooooo");
  /** 
  ...
  */
  ...
  const [ResponseBusquedaFirma, setResponseBusquedaFirma] =
    useState<InterfaceResponseBusquedaFirmas>();
  ...
  useEffect(() => {
    ...
    if (!ResponseBusquedaFirma) return;
        ...
        sketchWeb.on("create", function (event) {
          if (event.state === "complete") {
            ...
            if (ext) {
              ...
              //Lat, Long
              //Coordenadas Sup Izq
              const coordTopLeft = webMercatorUtils.xyToLngLat(
                ext.xmin,
                ext.ymax,
              );
              //Al state Latitud
              ...
              //Coordenadas Inf, Der
              const coordBotRight = webMercatorUtils.xyToLngLat(
                ext.xmax,
                ext.ymin,
              );
              //Al state Latitud
   ...
   */
  const enviarDispatchs = function (rows: any[]) {
  ...
  const showHideStates = (msg: string) => {
    alert("Widget Búsqueda Firmas Espectrales - REFA (2025-08-26) - Versión 1.0.0");
  }
