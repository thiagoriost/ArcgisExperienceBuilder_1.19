...
const FiltersSrcSIEC = function (
  {
    ...
    setCoberLst,
    showCoberFilt1,
    setShowCoberFilt1State,
    disCoberFilt1,
    setDisCoberFilt1State,
    showCoberFilt2,
    setShowCoberFilt2State,
    disCoberFilt2,
    setDisCoberFilt2State,
    selCoberFilt1Val,
    setSelCoberFilt1ValState,
    selCoberFilt2Val,
    setSelCoberFilt2ValState,
    coberFilt1Lst,
    ...
    setWidgetModules,
    chkValueHelp,
    setChkValueHelpState,
    catalBtnDis,
    setCatalBtnState,
  }:any)
{
  console.log(999999,"FiltersSrcSIEC")
  ...
  function componentDidUpdate() {
    if (props.state === "CLOSED") {
      ...
      if (radValueNav === "selArea") {
        ...
        if (jimuMapView) {
          jimuMapView.view.on("click", (evt:any) => {
            ...
            console.log(
              "Verif coordenada lat filtro =>",
              pointMap.latitude.toFixed(3),
            );
            console.log(
              "Verif coordenada lon filtro =>",
              pointMap.longitude.toFixed(3),
            );
  ...
  */
        ...
        fetch(urlServicioSIEC, {
          ...
          .then((rows) => {
            ...
            //Validador consumo por error del server (cód http <> 200 )
              ...
              console.error(
                "Error obteniendo data del server =>",
                jsonErr["errorMsg"],
              ) +
                " " +
                "(" +
          ...
          .then((data) => {
            ...
            console.log(
              "Contenido json desde petición =>",
              sortProyectos(data.data),
            );
        ...
        fetch(urlServicioSIEC, {
          ...
          .then((dataRowsCober) => {
            ...
            //Validador consumo por error del server (cód http <> 200 )
              ...
              console.error(
                "Error Obteniendo lista departamentos del server =>",
                dataRowsCober["errorMsg"],
              ) +
                " " +
                "(" +
                "código http =>" +
                dataRowsCober["errorCode"] +
                ")";
              throw (
                dataRowsCober["errorMsg"] +
                " " +
          ...
          .then((coberDataPrim) => {
            ...
            for (var cont = 0; cont < coberDataPrim.data.length; cont++) {
              jsonSIEC = {
                objectid:
                  coberDataPrim["data"][cont].Id_Valor_Dominio.toString(),
   ...
   */
  const getJSONCober1 = async function (
    selCoberVal: string,
    idControl: string = "",
  ) {
    //Objetos locales
    ...
    try {
      ...
      if (selCoberArr[0].toString() !== "*") {
        ...
        console.log(
          "Petición para control" + " " + idControl + " " + "=>",
          urlServicioSIEC,
        );
        coberSecData = await getDominioValor(
          tokenSeg["data"].access_token,
          urlServicioSIEC,
        );
        ...
        if (coberSecData["data"].length > 0) {
          ...
          for (
            var conCoberSec = 0;
            conCoberSec < coberSecData["data"].length;
            conCoberSec++
          ) {
            ...
            if (idControl === "Coberturas1") {
              ...
              console.log(
                "Test Num regs asociados al dominio " +
                  coberSecData["data"][conCoberSec].Id_Valor_Dominio +
                  " " +
                  "=>",
                await getNumRegsCober1(
                  coberSecData["data"][conCoberSec].Id_Valor_Dominio,
                ),
              );
              //Obtener número registros asociados al campo Nivel 3
              const numRegs = await getNumRegsCober1(
                coberSecData["data"][conCoberSec].Id_Valor_Dominio,
              );
              if (numRegs["data"].length > 0) {
                ...
                jsonData = {
                  objectid:
                    coberSecData["data"][
                      conCoberSec
                    ].Id_Valor_Dominio.toString(),
                  covertype:
                    coberSecData["data"][conCoberSec].Descripcion_Valor,
                };
            ...
            if (idControl === "Coberturas2") {
              ...
              const numRegs = await getNumRegsCoberN3(
                coberSecData["data"][conCoberSec].Descripcion_Valor,
              );
      ...
      console.log(
        "Error obteniendo coberturas secundarias del server =>",
        error,
      );
      throw error;
    }
  ...
  const getNumRegsCober1 = async function (selCoberVal: string) {
    ...
    urlServicioSIEC =
      urls.api_host + urls.api_getCoberSecTer + parseInt(selCoberVal);
  ...
  const getNumRegsCoberN3 = async function (selCoberTxt: string = "") {
    ...
    if (typeof selCoberTxt !== "undefined" || selCoberTxt !== "") {
      ...
      urlServicioSIEC = await getWhere(
        outFieldsService.fieldsOut,
        urls.firmasEspTCober,
        true,
        where,
        "",
        "",
   ...
   */
  const getJSONCober2 = async function (
    selCober1Val: string,
    idControl: string = "",
  ) {
  ...
  const getJSONDptos = async function () {
    ...
    const urlDivipolaDptos = await getWhere(
      "decodigo,denombre",
      urls.Departamentos,
      false,
      critSeleccDpto,
    ...
    try {
      await fetch(urlDivipolaDptos, {
        ...
        .then((DptosDataLst) => {
          ...
          console.log(
            "Array Lista Dptos sorted =>",
            sortDptos(DptosDataLst.features),
          );
        ...
        fetch(urlServicioSIEC, {
          ...
          .then((data) => {
            ...
            for (var cont = 0; cont < data.data.length; cont++) {
              ...
              if (
                (data.data[cont].Id_Proyecto == proyId || proyId === "*") &&
                data.data[cont].CampanaName != null
              ) {
                jsonSIEC = {
  ...
  const getWhere = async function (
    ...
    try {
      ...
      if (outStatistics && outStatistics.length > 0) {
        ...
        throw new Error(
          "Debe proporcionar OutFields o outStatistics o inputGeometry o Input Spatial Reference o Spatial Relationship",
        );
      }
      // Construir URL final
  ...
  function procesaDuplic(arrResult, opc) {
    ...
    switch (opc) {
      case "prj": {
        newArrResult = arrResult.filter(
          (obj, index, self) =>
            index === self.findIndex((t) => t.projectname === obj.projectname),
        );
        return newArrResult;
      }
      case "cam": {
        newArrResult = arrResult.filter(
          (obj, index, self) =>
            index === self.findIndex((t) => t.campananame === obj.campananame),
        );
        return newArrResult;
      }
      case "cov": {
        newArrResult = arrResult.filter(
          (obj, index, self) =>
            index === self.findIndex((t) => t.covertype === obj.covertype),
        );
        return newArrResult;
      }
      default: {
  ...
  const getJSONMpio = async function (idDpto) {
    ...
    urlDivipolaMpios = await getWhere(
    ...
    try {
      await fetch(urlDivipolaMpios, {
        ...
        .then((mpiosDataLst) => {
          ...
          console.log(
            "Array Mpios Sorted =>",
            sortMpios(mpiosDataLst.features),
          );
  ...
  function consultaCatal(evt: { preventDefault: () => void }) {
    ...
    if (
      ((typeof selCoberVal !== "undefined" &&
        typeof selCoberFilt1Val === "undefined" &&
        coberFilt1Lst.length === 0) ||
        (typeof selCoberFilt1Val !== "undefined" &&
          typeof selCoberFilt2Val !== "undefined" &&
          coberFilt2Lst.length === 0)) &&
      typeof dptoSel === "undefined" &&
      typeof mpioSel === "undefined" &&
      txtValorLatSuIz.trim() === "" &&
      txtValorLatInDe.trim() === "" &&
      txtValorLonSuIz.trim() === "" &&
      txtValorLonInDe.trim() === "" &&
      txtValorLat.trim() === "" &&
      txtValorLon.trim() === "" &&
      typeof selProyVal === "undefined" &&
      typeof selCampaVal === "undefined"
    ) {
    ...
    props.dispatch(
      appActions.widgetStatePropChange(
        "widget_81",
        "dataFromDispatchWidget_searchSIEC",
        dataToRenderTablaResultados,
      ),
    );
  ...
  const getJSONFilter = async function () {
    ...
    if (typeof selCoberVal !== "undefined") {
      ...
      //Procesamiento Cobertura => Nivel 1, cuando coberturas 1 => Nivel 2 no es seleccionado y tiene lista definida
        ...
        const tokenSeg = tokSegObj["data"].access_token;
        urlApiServicioSIEC =
          urls.api_host + urls.api_getCoberTerInBNE + paramNiv2Str;
        //console.log ("Petición API =>",urlApiServicioSIEC);
        try {
          var coberNiv3Obj = await getDominioValor(
            tokenSeg,
            urlApiServicioSIEC,
          );
          ...
          for (
            var contNiv3 = 0;
            contNiv3 < coberNiv3Obj["data"].length;
            contNiv3++
          ) {
            ...
            if (typeof paramNiv3Str === "undefined") {
              paramNiv3Str =
                "'" +
                coberNiv3Obj["data"][contNiv3].Descripcion_Valor +
                "'" +
                ",";
            } else {
              paramNiv3Str +=
          ...
          //Generación consulta
          where =
            "tipo_cobertura" + " " + "IN" + " " + "(" + paramNiv3Str + ")";
        } catch (error) {
      ...
      //Procesamiento coberturas 1 => Nivel 2, cuando coberturas 2 => Nivel 3 no es seleccionado y tiene lista definida
        ...
        try {
          ...
          for (var contNiv3 = 0; contNiv3 < coberFilt2Lst.length; contNiv3++) {
            ...
            if (typeof paramNiv3Str === "undefined") {
              paramNiv3Str =
                "'" + coberFilt2Lst[contNiv3].covertype + "'" + ",";
            } else {
              paramNiv3Str +=
                "'" + coberFilt2Lst[contNiv3].covertype + "'" + ",";
            }
      ...
      //Procesamiento cobertura => Nivel 1 - texto del control
        ...
        if (
          selCoberFilt1Val.split(";")[0] === "*" ||
          coberFilt1Lst.length === 1
        ) {
    ...
    if (typeof dptoSel !== "undefined") {
      if (typeof where !== "undefined") {
        if (where.length > 0 && dptoSel !== "*") {
          where =
            where + " " + "AND" + " " + "codigo_departamento='" + dptoSel + "'";
        } else if (dptoSel !== "*") {
    ...
    if (typeof mpioSel !== "undefined") {
      if (typeof where !== "undefined") {
        if (where.length > 0 && mpioSel !== "*") {
          where =
            where + " " + "AND" + " " + "codigo_municipio='" + mpioSel + "'";
        } else if (mpioSel !== "*") {
    ...
    if (radValueNav === "navMap") {
      ...
      if (
        typeof txtValorLat !== "undefined" &&
        txtValorLat !== "" &&
        typeof txtValorLon !== "undefined" &&
        txtValorLon !== ""
      ) {
    ...
    //parametro Input Geometry
      ...
      if (
        typeof txtValorLatSuIz !== "undefined" &&
        txtValorLatSuIz !== "" &&
        typeof txtValorLatInDe !== "undefined" &&
        txtValorLatInDe !== "" &&
        typeof txtValorLonSuIz !== "undefined" &&
        txtValorLonSuIz !== "" &&
        typeof txtValorLonInDe !== "undefined" &&
        txtValorLonInDe !== ""
      ) {
        geomCoords =
          txtValorLonSuIz +
    ...
    if (typeof selProyVal !== "undefined") {
      ...
      if (typeof where !== "undefined") {
        if (where.length > 0 && selProyVal !== "*") {
          where = where + " " + "AND" + " " + "projectnam='" + selProyVal + "'";
        } else if (selProyVal !== "*") {
    ...
    if (typeof selCampaVal !== "undefined") {
      ...
      //else if (selCampaVal === '*' && selProyVal === '*' && typeof selCoberVal === 'undefined')
        ...
        (selCampaVal === "*" || typeof selCampaVal === "undefined") &&
        (selProyVal === "*" || typeof selProyVal === "undefined") &&
        typeof selCoberVal === "undefined"
      ) {
        where = "1=1";
      }
    ...
    if (
      typeof selCoberVal === "undefined" &&
      (dptoSel === "*" || typeof dptoSel === "undefined") &&
      (mpioSel === "*" || typeof mpioSel === "undefined") &&
      (selProyVal === "*" || typeof selProyVal === "undefined") &&
      (selCampaVal === "*" || typeof selCampaVal === "undefined")
    ) {
      where = "1=1";
    ...
    //Se usa la petición para paso de parámetros
    const urlServicioSIEC = await getWhere(
      outFieldsService.fieldsOut,
      urls.firmasEspTCober,
      true,
      where,
   ...
   */
  const getSelectedDataFilter = async function (
    urlServicioSIEC: string,
    opc: string = "",
  ) {
    if (opc === "") {
        ...
        await fetch(urlServicioSIEC, {
          ...
          .then((dataJSON) => {
            ...
            if (typeof dataJSON["error"] !== "undefined") {
              ...
              console.error(
                "Error obteniendo data asociado al filtro de consulta del Mapserver (" +
                  urlServicioSIEC +
                  ") =>",
                jsonErr["errorMsg"] +
                  " " +
                  "(" +
                  "Detalle error asociado => " +
                  jsonErr["errorMsgDet"] +
                  ")" +
                  " " +
                  "(" +
        ...
        await fetch(urlDivipolaMpios, {
          ...
          .then((rows) => {
            ...
            //Validador consumo por error del server (cód http <> 200 )
              ...
              console.error(
                "Error obteniendo lista Mpios del server =>",
                jsonErr["errorMsg"],
              ) +
                " " +
                "(" +
                jsonErr["errorDetails"] +
                ")" +
                " " +
                "(" +
                "código http =>" +
          ...
          .then((data) => {
            ...
            if (typeof data["error"] === "undefined") {
              ...
              for (var cont = 0; cont < dataJSON.length; cont++) {
                ...
                if (
                  dataJSON[cont].attributes.codigo_municipio !== null &&
                  dataJSON[cont].attributes.codigo_departamento !== null
                ) {
                  //Copia del código Dpto
                  codDptoDivipola =
                    dataJSON[cont].attributes.codigo_departamento;
                  ...
                  for (
                    var contMpio = 0;
                    contMpio < data.features.length;
                    contMpio++
                  ) {
                    //Caso especial, búsqueda departamento con código 11 asociado Bogotá
                    if (
                      codDptoDivipola === codDeptoDivip.codDepto &&
                      data.features[contMpio].attributes.mpcodigo ===
                        codMpioDivipola
                    ) {
                      //Asignación nombre departamento por su codigo
                      dataJSON[cont].attributes.codigo_departamento =
                        data.features[contMpio].attributes.mpnombre;

                      //Asignación nombre municipio por su código
                      dataJSON[cont].attributes.codigo_municipio =
                        data.features[contMpio].attributes.mpnombre;
                    }
                    //Municipios
                    else if (
                      data.features[contMpio].attributes.decodigo ===
                        codDptoDivipola &&
                      data.features[contMpio].attributes.mpcodigo ===
                        codMpioDivipola
                    ) {
                      //Asignación nombre departamento por su codigo
                      dataJSON[cont].attributes.codigo_departamento =
                        data.features[contMpio].attributes.depto;
            ...
            //Data emulada para Data Grid - 2025-08-28
              ...
              console.error(
                "Error código =>" + " " + (data as any)?.error?.code,
              );
   ...
   */
  const limpiarControlesFilter = function (evt: {
    preventDefault: () => void;
  }) {
    evt.preventDefault();
    //Limpieza controles
  ...
  const renderStyledLabel = (text: string) => {
    if (entorno === "dev") {
      return <Label size="default">{text}</Label>;
    }

    return (
      <Label
        size="default"
        className="app-root-emotion-cache-ltr-h9i97z_prod"
      >
        {text}
      </Label>
    );
  ...
  return (
    <form onSubmit={consultaCatal}>
      <div className="mb-1">
        {renderStyledLabel(entorno === "dev" ? "Nivel 11" : "Nivel 111")}
        <Select
        ...
        >
          {coberLst.map((option) => (
            <option value={option.objectid + ";" + option.covertype}>
              {option.covertype}
            </option>
          ))}
        </Select>
        ...
        <div className="mb-1">
          ...
          >
            {coberFilt1Lst.map((valuesCober1) => (
              <option
                value={valuesCober1.objectid + ";" + valuesCober1.covertype}
              >
                {valuesCober1.covertype}
              </option>
            ))}
          </Select>
        </div>
        ...
        <div className="mb-1">
          ...
          >
            {coberFilt2Lst.map((valuesCober2) => (
              <option
                value={valuesCober2.objectid + ";" + valuesCober2.covertype}
              >
                {valuesCober2.covertype}
              </option>
            ))}
          </Select>
        </div>
      ...
      <div className="mb-1 proyCampaSecc">
        ...
        >
          {jsonDpto.length > 0 &&
            jsonDpto.map((dptoItem) => (
              <option value={dptoItem.attributes.decodigo}>
                {dptoItem.attributes.denombre}
              </option>
            ))}
        </Select>
      </div>
      <div className="mb-1">
        ...
        >
          {mpioLst.length > 0 &&
            mpioLst.map((mpioItem) => (
              <option value={mpioItem.attributes.mpcodigo}>
                {mpioItem.attributes.mpnombre}
              </option>
            ))}
        </Select>
      </div>
      <div className="mb-1 proyCampaSecc">
        ...
        >
          {proyLst.length > 0 &&
            proyLst.map((option) => (
              <option value={option.projectname + ";" + option.objectid}>
                {option.projectname}
              </option>
            ))}
        </Select>
      ...
      <div role="radiogroup" className="alignRad">
        ...
        <Label className="d-flex alignLab2">
          <img
            ...
            height="15px"
            className="helpIcn"
            src={`${pathDataGridSIEC.path}/images/ayudaBuscFE.png`}
      ...
      <div className="btns">
        ...
        <Button size="default" type="default" onClick={limpiarControlesFilter}>
          Limpiar
        </Button>
      </div>
